...
class XheBrowserCompatible extends XheBaseObject
{
	...
	// добавить плагины для селениум моделей (через .crx файлы)
	function add_plugins($paths_to_plugins,$refresh=true)
	{
		global $selenium;
		return $selenium->add_plugins($paths_to_plugins,$refresh);
	}	
	// получить WebDriver Selenium SessionID 
	function get_webdriver_session()
	{
		global $selenium;
		return $selenium->get_webdriver_session();
	}
