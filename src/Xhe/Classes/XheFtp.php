...
class XheFtp extends XheFtpCompatible
{
	...
	{
	   	$params = array( "server" => $server, "folder" => $folder );		
	    	$resw = $this->call_get(__FUNCTION__,$params);
		if ($res!="")
			return explode("\n",$res);
		else
			return null;
	...
	function is_directory_exists($server,$dir_name)
	{
	...
	// очистить папку на сервере
	function clear_directory($server,$dir_name)
	{
	   	$params = array( "server" => $server , "dir_name" => $dir_name );
	    	return $this->call_boolean(__FUNCTION__,$params);
	}
	...
	function is_file_exists($server,$remote_file)
	{
	   	$params = array( "server" => $server , "remote_file" => $remote_file );
	    	return $this->call_boolean(__FUNCTION__,$params);
	}
