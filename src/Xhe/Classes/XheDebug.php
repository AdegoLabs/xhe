...
class XheDebug extends XheBaseObject
{
	...
	// получить число ресурсов, использованных программой (0 - GDI, 1 - USER ,  2 - бывший максимум GDI , 4 - бывший максимум USER , 5 - число HANDLE использованых програмой)
	...
	// получить id процесса программы
	...
	// показать или спрятать дополнительную панель отладки
	function show_extended_debug_pane($is_show = true)
	{
		$params = array( "is_show" => $is_show );
		return $this->call_boolean(__FUNCTION__,$params);
	}
	// задать позицию дополнительного окна отладки
	function set_extended_debug_pane_pos($x,$y,$width,$height)
	{
		$params = array( "x" => $x , "y" => $y, "width" => $width , "height" => $height );
		return $this->call_boolean(__FUNCTION__,$params);
	}
	// задать прозрачность дополнителоьной панели отладки
	function set_extended_debug_pane_opacity($opacity=0.4)
	{
		$params = array( "opacity" => $opacity );
		return $this->call_boolean(__FUNCTION__,$params);
	}
	// задать цвета дополнительной панели отладки
	function set_extended_debug_pane_colors($backColor,$foreColor)
	{
		$params = array( "backColor" => $backColor , "foreColor" => $foreColor );
		return $this->call_boolean(__FUNCTION__,$params);
	}
