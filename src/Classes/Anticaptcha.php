...
class Anticaptcha extends XheBaseCaptcha {
	...
	{
        return $this->reportIncorrectImageCaptcha($id);    
	}
	// report bug capcha
    ...
    public function reportIncorrectImageCaptcha($id)
    {
        $postData = array(
            "clientKey" =>  $this->clientKey,
            "taskId"    =>  $id
        );
        $result = $this->jsonPostRequest("reportIncorrectImageCaptcha", $postData);
        if ($result == false) {
            $this->debout("API error", "red");
            return false;
        }
        ...
        else if($result->errorId == 16)
        {
            return 'error 16 - captcha not found';     
        }
        else {
            return false;
        }
